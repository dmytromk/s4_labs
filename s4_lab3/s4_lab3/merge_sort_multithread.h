...
void mergesort_do_multithread(const main_iter left, const main_iter right, const auxil_iter aux_left, int cur_thread_amount, int max_threads)
{
	...
	if (cur_thread_amount < max_threads-1)
	{
		...
		std::thread th1(&mergesort_do_multithread<main_iter, auxil_iter>,
			left, middle, aux_left, cur_thread_amount, max_threads);

		std::thread th2(&mergesort_do_multithread<main_iter, auxil_iter>,
	...
	{
		mergesort_do_multithread(left, middle, aux_left, cur_thread_amount, max_threads);
		mergesort_do_multithread(middle, right, std::next(aux_left, std::distance(left, middle)), cur_thread_amount, max_threads);
	}
...
void mergesort_multithread(const main_iter left, const main_iter right, int max_threads = 8) //max amount of threads MUST BE EVEN
{
	...
	mergesort_do_multithread(left, right, buffer.begin(), 0, max_threads);
}